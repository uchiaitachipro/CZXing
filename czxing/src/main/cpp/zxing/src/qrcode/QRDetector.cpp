...
*/
static float
SizeOfBlackWhiteBlackRun(const BitMatrix &image, int fromX, int fromY, int toX, int toY) {
...
*/
static float
SizeOfBlackWhiteBlackRunBothWays(const BitMatrix &image, int fromX, int fromY, int toX,
                                 int toY) {
...
*/
static float CalculateModuleSizeOneWay(const BitMatrix &image, const ResultPoint &pattern,
...
*/
static float CalculateModuleSize(const BitMatrix &image, const ResultPoint &topLeft,
                                 const ResultPoint &topRight,
                                 const ResultPoint &bottomLeft) {
    // Take the average
    return (CalculateModuleSizeOneWay(image, topLeft, topRight) +
            CalculateModuleSizeOneWay(image, topLeft, bottomLeft)) / 2.0f;
}
...
*/
AlignmentPattern
FindAlignmentInRegion(const BitMatrix &image, float overallEstModuleSize, int estAlignmentX,
    ...
    return StrictAlignmentPatternFinder::Find(image, alignmentAreaLeftX, alignmentAreaTopY,
...
CreateTransform(const ResultPoint &topLeft, const ResultPoint &topRight,
                const ResultPoint &bottomLeft, const AlignmentPattern &alignmentPattern,
                int dimension) {
...
*/
static int ComputeDimension(const ResultPoint &topLeft, const ResultPoint &topRight,
                            const ResultPoint &bottomLeft, float moduleSize) {
    int tltrCentersDimension = RoundToNearest(
            ResultPoint::Distance(topLeft, topRight) / moduleSize);
    int tlblCentersDimension = RoundToNearest(
            ResultPoint::Distance(topLeft, bottomLeft) / moduleSize);
...
ProcessFinderPatternInfo(const BitMatrix &image, const FinderPatternInfo &info,
    ...
    int dimension = ComputeDimension(info.topLeft, info.topRight, info.bottomLeft,
                                     moduleSize);
    if (dimension < 0)
        return {};
    ...
    if (!provisionalVersion->alignmentPatternCenters().empty()) {
        ...
        int estAlignmentX = static_cast<int>(info.topLeft.x() + correctionToTopLeft *
                                                                (bottomRightX -
                                                                 info.topLeft.x()));
        int estAlignmentY = static_cast<int>(info.topLeft.y() + correctionToTopLeft *
                                                                (bottomRightY -
                                                                 info.topLeft.y()));
    ...
    PerspectiveTransform transform = CreateTransform(info.topLeft, info.topRight,
                                                     ...
                                                     dimension);
    auto sampler = GridSampler::Instance();
    sampler->setHookFunction(f);
    auto bits = sampler->sampleGrid(image, dimension, dimension, transform);
    if (bits.empty())
        return {};

    if (alignmentPattern.isValid())
        return {std::move(bits),
                {info.bottomLeft, info.topLeft, info.topRight, alignmentPattern}};
    else
        return {std::move(bits), {info.bottomLeft, info.topLeft, info.topRight}};
}

DetectorResult Detector::Detect(const BitMatrix &image, bool tryHarder, HookFunction f) {
    FinderPatternInfo info = AntFinderPatternFinder::Find(image, tryHarder,f);

    if (!info.isValid())
        return {};

    return ProcessFinderPatternInfo(image, info, f);
}
